...
export default function Reg({ federalDists, regions, municipals }) {
  ...
  return (
    <>
      <h1 className='text-info'>Регистрация</h1>
      {error && <p style={{ color: 'red' }}>{error}</p>}
      <form onSubmit={submitHandler}>
        <div className="mb-3">
          <label htmlFor="exampleInputName" className="form-label text-info">ФИО</label>
        ...
        <select name="federalDist" className="form-select form-select-lg mb-3 text-info" aria-label=".form-select-lg example">
          <option value="" disabled selected>Федеральный округ</option>
          {federalDists?.map((el) => <option key={el.id} value={el.id}>{el.name}</option>)}
        </select>
        <select name="region" className="form-select form-select-lg mb-3 text-info" aria-label=".form-select-lg example">
          <option value="" disabled selected>Регион</option>
          {regions?.map((el) => <option key={el.id} value={el.id}>{el.name}</option>)}
        </select>
        <select name="municipal" className="form-select form-select-lg mb-3 text-info" aria-label=".form-select-lg example">
